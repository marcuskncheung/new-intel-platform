        ...
        for cp in case_profiles:
            ...
            if hasattr(cp, 'surveillance_id') and cp.surveillance_id:
                ...
                if surveillance:
                    ...
                    # 🔧 FIX: Search in standardized fields first
                    if surveillance.alleged_subject_english and query_lower in surveillance.alleged_subject_english.lower():
                        match_found = True
                        match_reason.append(f"Surveillance Person: {surveillance.alleged_subject_english.split(',')[0].strip()}")
                    if surveillance.alleged_subject_chinese and query_lower in surveillance.alleged_subject_chinese.lower():
                        match_found = True
                        match_reason.append(f"Surveillance Person (CN): {surveillance.alleged_subject_chinese.split(',')[0].strip()}")
                    ...
                    if surveillance.details_of_finding and query_lower in surveillance.details_of_finding.lower():
                        match_found = True
            ...
            if match_found:
                # Remove duplicates from match_reason while preserving order
                seen = set()
                unique_reasons = []
                ...
                results.append({
                    ...
                    'match_reason': ', '.join(unique_reasons[:3]),  # Limit to 3 reasons for readability
                    'date_created': cp.date_of_receipt.strftime('%Y-%m-%d') if cp.date_of_receipt else 'N/A'
                })
        
        # Sort results by INT reference (most recent first)
        results.sort(key=lambda x: x['int_reference'], reverse=True)
...
def int_analytics():
    ...
    try:
        ...
        total_sources = total_emails + total_whatsapp + total_online + total_surveillance + total_received_by_hand
        avg_items_per_int = total_sources / total_ints if total_ints > 0 else 0
...
@app.route("/received_by_hand/<int:entry_id>/update_int_reference", methods=["POST"])
@login_required
def update_received_by_hand_int_reference(entry_id):
    """Update INT reference for a Received by Hand entry"""
    entry = db.session.get(ReceivedByHandEntry, entry_id)
    if not entry:
        flash("Entry not found.", "error")
        return redirect(url_for('int_source'))
...
@app.route("/received_by_hand/<int:entry_id>/update_details", methods=["POST"])
@login_required
def update_received_by_hand_details(entry_id):
    """Update details for a Received by Hand entry (WhatsApp-style form)"""
    entry = db.session.get(ReceivedByHandEntry, entry_id)
    if not entry:
        flash("Entry not found.", "error")
        return redirect(url_for('int_source'))
...
@app.route("/received_by_hand/document/<int:document_id>/delete", methods=["POST"])
@login_required
def delete_received_by_hand_document(document_id):
    """Delete a document from a Received by Hand entry"""
    doc = db.session.get(ReceivedByHandDocument, document_id)
    if not doc:
        flash("Document not found.", "error")
        return redirect(url_for('int_source'))
    
    entry_id = doc.received_by_hand_id
    try:
        db.session.delete(doc)
        db.session.commit()
